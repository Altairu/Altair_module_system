...
class AltairCanTool:
    def __init__(self, root):
        self.root = root
        self.root.title("Altair MDD Controller")
        ...
        self.chan_var = tk.StringVar(value="COM3")
    ...
    def _setup_style(self):
        ...
        style.configure("TLabelframe.Label", background="#111A2C", foreground="#8EC5FF", font=("Segoe UI", 10, "bold"))
        style.configure("TLabel", background="#111A2C", foreground="#DDE6F6")
        style.configure("Header.TLabel", background="#0B1220", foreground="#DDE6F6", font=("Segoe UI", 16, "bold"))
        style.configure("Sub.TLabel", background="#0B1220", foreground="#8BA3C7")
    ...
    def _build_ui(self):
        ...
        ttk.Label(main, text="Windows USB-to-CAN | 1 Mbps fixed | 4 Motors full control", style="Sub.TLabel").pack(anchor="w", pady=(0, 10))
    ...
    def _build_connection_panel(self, parent):
        frame = ttk.LabelFrame(parent, text="CAN Connection")
        ...
        ttk.Combobox(
            ...
            values=["slcan", "pcan", "kvaser", "ixxat", "vector"],
            state="readonly",
            width=12,
        ).grid(row=0, column=1, padx=4)

        ttk.Label(frame, text="Channel / COM").grid(row=0, column=2, padx=8)
        ttk.Entry(frame, textvariable=self.chan_var, width=12).grid(row=0, column=3, padx=4)

        ttk.Label(frame, text=f"Bitrate: {CAN_BITRATE:,} bps (fixed)").grid(row=0, column=4, padx=10)
    ...
    def _build_param_panel(self, parent):
        ...
        defaults = [
            {"p": 1.50, "i": 0.10, "d": 0.05, "dia": 100, "inv": False},
            {"p": 1.50, "i": 0.10, "d": 0.05, "dia": 100, "inv": False},
            {"p": 1.50, "i": 0.10, "d": 0.05, "dia": 100, "inv": False},
            {"p": 1.50, "i": 0.10, "d": 0.05, "dia": 100, "inv": False},
        ]
        ...
        for idx, can_id in enumerate(PARAM_CAN_IDS):
            row = idx + 1
            values = defaults[idx]
            p_var = tk.DoubleVar(value=values["p"])
            i_var = tk.DoubleVar(value=values["i"])
            d_var = tk.DoubleVar(value=values["d"])
            dia_var = tk.IntVar(value=values["dia"])
            inv_var = tk.BooleanVar(value=values["inv"])
