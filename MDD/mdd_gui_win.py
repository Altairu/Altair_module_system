...
# -*- coding: utf-8 -*-
"""
MDD（モータードライブ）制御 GUI ツール（Windows対応）
状態連動 CAN プロトコル対応:
...
class CANBackend:
    ...
    def _build_can_bus(self) -> bool:
        ...
        try:
            ...
            self._log(f"✅ {self.interface} で接続: {self.channel} @ {self.bitrate} bps")
            return True
...
class AltairGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("MDD 制御 GUI (Windows対応)")
    ...
    def _build_ui(self):
        # ─── 接続パネル ─────────────────────────
        conn_frame = ttk.LabelFrame(self.root, text="CAN 接続 (Windows用)", padding=10)
        ...
        ttk.Entry(conn_frame, textvariable=self.var_bitrate, width=10).grid(row=0, column=5, padx=5)
    ...
    def _on_connect(self):
        try:
            ...
            self.backend = CANBackend(interface, channel, bitrate, self.log_queue)
