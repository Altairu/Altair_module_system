...
class ServoCanGUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("0x100 サーボ指令 CAN送信ツール  (10ms周期)")
    ...
    def _build_connection_frame(self, parent):
        frm = ttk.LabelFrame(parent, text="CAN接続設定", padding=6)
        ...
        ttk.Combobox(frm, textvariable=self.var_iface, width=14,
                     values=["socketcan", "slcan", "pcan", "kvaser", "vector", "ixxat"]
